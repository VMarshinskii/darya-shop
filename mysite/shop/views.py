...
def order(request):
    ...
    if request.user.is_authenticated():
        if request.POST:
            ...
            if form.is_valid():
                ord = additions.create_order(request, request.user)
                user = additions.update_user(request) or request.user
                return render_to_response("order_thanks.html")
            else:
                args['form'] = form
                return render_to_response("order_registered.html", args)
        model = additions.get_model_order(request)
    ...
    if request.POST:
        form = OrderForm(request.POST)
        if form.is_valid():
            password = additions.random_str(7)
            user = additions.create_user(request, password)
            ord = additions.create_order(request, user)
...
def add_in_cart(request, id=-1):
    if "user_cart" in request.session:
        ...
        products = additions.unserialize(user_cart.products)
        products[int(id)] = products.get(int(id), 0) + 1
        user_cart.products = additions.serialize(products)
        user_cart.save()
    else:
        user_cart = UserCart()
